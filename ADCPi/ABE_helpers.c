...
char read_byte_data(char address, char reg) {
...
char read_byte_array(char address, char reg, char length) {
	...
	if (read(i2cbus, buf, length) != 1) { // Read back data into buf[]
		printf("Failed to read from slave\n");
		exit(1);
	}
...
void write_single_byte(char address, char value) {
	if ((i2cbus = open(fileName, O_RDWR)) < 0) {
		printf("Failed to open i2c port for write\n");
		exit(1);
	}
